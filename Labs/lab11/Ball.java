...
public class Ball { 
    ...
    public void bounce(Box wall) {
        if (vy < 0 && wall.contains(x, y-radius))
            vy *= -1; // moving downward and bottommost point of this ball touched wall
        else if (vy > 0 && wall.contains(x, y+radius))
            vy *= -1; // moving upward and topmost point of this ball touched wall
        if (vx < 0 && wall.contains(x-radius, y))
            vx *= -1; // moving leftward and leftmost point of this ball touched wall
        else if (vx > 0 && wall.contains(x+radius, y))
            vx *= -1; // moving rightward and rightmost point of this ball touched wall
    }

    public void gravity(double duration) {
        this.vy -= duration * 9.8 / 10000;
    }
